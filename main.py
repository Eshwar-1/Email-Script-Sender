...
app.title("E-Mail Sender | Eshwar")
            ...
            if sender == "" or app_password == "" or not receivers or subject_details == "" or body_message == "":
                ...
                for receiver_mail in receivers:
                    try:
                        ...
                        print(Fore.RED + f"Failed to send email to {receiver_mail.strip()}. Error: {e}" + Fore.RESET)
                server.quit()
...
label = Label(app, text="Email App - Eshwar", font=heading_style, fg='blue')
...
Button(app, text="Reset", font=font_bold_style, bg='red', fg='white', width=10, command=reset).grid(row=7, column=1, sticky=W, pady=20)
Button(app, text="Attach File", font=font_bold_style, bg='yellow', command=attach_file).grid(row=7, column=1, sticky=E, padx=5)
